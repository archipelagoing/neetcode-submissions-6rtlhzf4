...
# sent over network, & decoded back into original list of strings
    def decode(self, s: str) -> List[str]:
        ...
        while s[i] != '#':
            ...
            while s[i] != ',':
                cur += s[i]
                i += 1 # read until the commas, extract the int
            sizes.append(int(cur)) # append the int
            i += 1 # go to the next character, and continue the loop

        i += 1 # jump over the # sign.

        for chunk in sizes:
            res.append(s[i:(i + chunk)])
            i += chunk
